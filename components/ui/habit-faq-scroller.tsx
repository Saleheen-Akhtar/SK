...
export const FaqCard = ({ question, answer }: { question: string; answer: string }) => {
  return (
    <div className="flex flex-col items-start gap-4 p-6 bg-white/90 backdrop-blur-md rounded-xl shadow-lg w-96 flex-shrink-0 faq-card border border-amber-200/50">
      <h3 className="text-xl font-bold text-amber-950 faq-title">{question}</h3>
      <p className="text-lg text-amber-900/80 faq-answer">{answer}</p>
    </div>
  );
};
