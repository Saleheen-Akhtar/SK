...
<head>
  ...
  <!-- Self-hosted fonts loaded via foundations.css -->
...
<?php if ($ann_countdown): ?>
<!-- Announcement bar scripts enqueued via setup.php -->
<?php endif; ?>
<?php endif; ?>
...
<nav class="sk-sidenav" id="sk-sidenav"
  ...
  <ul class="sk-sidenav-links">
    <?php if (empty($sk_nav_items)) : ?>
      <?php if (current_user_can('manage_options')) : ?>
        <li class="sk-nav-setup-prompt" style="padding: 10px 0;">
          <a href="<?php echo esc_url(admin_url('edit.php?post_type=sk_nav')); ?>" style="color: var(--gold); border: 1px dashed var(--gold); padding: 8px 12px; font-size: 11px; text-align: center; display: block; border-radius: 4px;">
            <?php esc_html_e('Navigation not configured', 'sacred-kompass'); ?>
          </a>
        </li>
      <?php endif; ?>
    <?php else : ?>
  ...
  <?php else : ?>
  <div class="sk-sidenav-cta">
    <a href="<?php echo esc_url( sk_option('contact_url', home_url( '/#contact' )) ); ?>" class="btn btn-primary sk-sidenav-cta-btn">
